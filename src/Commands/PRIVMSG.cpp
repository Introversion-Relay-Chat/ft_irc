...
std::string PRIVMSG(const Message &message, User *sender) {
	...
	// ERR_NORECIPIENT
	if (!recipient.size()) {
		return join(sender_prefix, "411", target, ERR_NORECIPIENT(message.command));
	}
	// ERR_NOTEXTTOSEND
	if (!message.trailing.length()) {
		return join(sender_prefix, "412", target, ERR_NOTEXTTOSEND());
	}
	...
	for (std::vector<std::pair<std::string, std::string> >::iterator it = responses.begin();it != responses.end();it++) {
		if (it->first == "401") {
			sender->getServer()->sendMsg(join(sender_prefix, "401", target, ERR_NOSUCHNICK(it->second)), sender);
		} else if (it->first == "404") {
			sender->getServer()->sendMsg(join(sender_prefix, "404", target, ERR_CANNOTSENDTOCHAN(it->second)), sender);
		} else {
