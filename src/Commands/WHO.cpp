...
std::string joinedChannelToString(std::set<std::string> joined_channels) {
	std::string result = "";
	for (std::set<std::string>::iterator it = joined_channels.begin(); it != joined_channels.end(); ++it) {
		result += *it + ", ";
	}
	if (result.length() > 0) {
	...
	if (message.middle.size() < 1 || (message.middle.size() == 1 && (message.middle[0] == "*" || message.middle[0] == "0"))) {
		...
		for (std::map<int, User *>::iterator it = users.begin(); it != users.end(); it++) {
			...
			server->sendMsg(
						...
						RPL_WHOREPLY(joinedChannelToString(target_joined_channels), target_user->getUsername(),
